...
public class SyntaxAnalyzer {
    ...
    public void run() {
        ...
        Status currentStatus = table.getInit();
        allStatus.add(currentStatus);
        Iterator<Token> iterator = tokens.iterator();
        Token token = null;
        boolean NoReduce = true;
        while (true) {
            if (iterator.hasNext() && NoReduce) {
                token = iterator.next();
            } else if (!NoReduce) {
                NoReduce = true;
            } else {
            ...
            action = table.getAction(currentStatus, token);
            if (action.getKind() == Action.ActionKind.Shift) {
                ...
                allStatus.subList(allStatus.size() - production.body().size(), allStatus.size()).clear();
                currentStatus = table.getGoto(allStatus.get(allStatus.size() - 1), production.head());
                allStatus.add(currentStatus);
                NoReduce = false;
